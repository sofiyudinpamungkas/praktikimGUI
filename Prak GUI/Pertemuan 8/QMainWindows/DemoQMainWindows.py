...
class MainForm(QMainWindow):
    ...
    def confirmation(self):
        if self.textEdit.document().isModified(): response = QMessageBox.question(self,
        'Konfirmasi',
        'Teks telah dimodifikasi. Simpan?')
        if response == QMessageBox.Yes: self.fileSaveActionTriggered()
    ...
    def fileSaveActionTriggered(self):
        if self.currentFileName == '': # mengeksekusi aksi Save As
